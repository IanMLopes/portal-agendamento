<template>
  <div v-if="horarios.length > 0">
    <p class="lead" v-if="horarios.length > 0">
      ...
      <b>Exame:</b> {{especialidade.descricao}} {{ (adicional != undefined && adicional.descricao != undefined && adicional.descricao != especialidade.descricao) ? `, ${adicional.descricao}` : `` }}</p>
    ...
    <div class="w-100 m-0 p-0 justify-content-start">
      <p class="txt-dark" style="font-size:12px;"><img src="../../assets/img/square-solid.svg" style="width:12px;"> Dias disponíveis</p>
    </div>
    <b-col md="auto">
      <b-calendar 
      label-help = ''
      ...
      <b-container class="d-flex">
          <div class="row p-0">
            ...
            @click="selecionarHorario(horario)"
             ...
             <b> {{horario.data.split(' ')[1].slice(0, -3)}}</b>
             </span>
             <span 
             class="border p-1"
...
export default {
   name: "cardsHorarios",
  props: ["horarios", "profissional", "especialidade", "adicional"],
  data: () => ({
    typeLoader: loader,
    isLoading: false,
    horarioSelecionado:'',
