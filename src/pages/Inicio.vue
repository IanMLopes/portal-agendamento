  ...
  <div class="content">
    <div class="md-layout">
      ...
      <div class="md-layout-item md-medium-size-100 md-size-70">
        <menu-principal-mobile v-if="dispositivo" data-background-color="green"></menu-principal-mobile>
        <menu-principal v-else data-background-color="green"></menu-principal>
      </div>
    </div>
  </div>
</template>
...
import MenuPrincipalMobile from "../pages/Layout/MenuPrincipalMobile.vue"
...
export default {
  ...
  mounted(){
    ...
    setTimeout(() => {
        this.carregarDados()
        if(this.$session.get('reload') == 1){
          this.$session.set('reload', 0)
          location.reload()
        }
      }, 1000);
  ...
  methods: {
    ...
    this.isLoading = true
      ...
      this.axios.get('/Cliente/Obter', { headers: headers })
        .then((response) => {

          this.dataPatient = response.data.dados
